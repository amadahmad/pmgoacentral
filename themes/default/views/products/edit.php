                            ...
                            if (!empty($warehouses) || !empty($warehouses_products)) {
                                ...
                                if(!empty($warehouses_products)) {
                                    foreach($warehouses_products as $wh_pr) {
                                        echo '<span class="bold text-info">'.$wh_pr->name.': <span class="padding05" id="rwh_qty_'.$wh_pr->id.'">'.$wh_pr->quantity.'</span>'.($wh_pr->rack ? ' (<span class="padding05" id="rrack_'.$wh_pr->id.'">'.$wh_pr->rack.'</span>)': '').'</span><br>';
                                    } 
                                }
                                echo '<div id="show_wh_edit"><div class="alert alert-danger margin010"><p>'.lang('edit_quantity_not_recommended_here').'<input type="hidden" value="0" name="warehouse_quantity" id="warehouse_quantity"></p></div>';
                                    ...
                                    <tbody><?php
                                        if ($this->input->post('attributes')) {
                                            ...
                                            foreach ($product_options as $option) {
                                                ...
                                                echo '<tr class="attr"><td><input type="hidden" name="attr_name[]" value="' . $option->attribute . '"><span>' . $option->attribute . '</span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value="' . $option->warehouse_id . '"><input type="hidden" name="attr_wh_name[]" value="' . $option->wh_name . '"><span>' . $option->wh_name . '</span></td><td class="quantity text-center"><input type="hidden" name="attr_quantity[]" value="' . $option->quantity . '"><span>' . $option->quantity . '</span></td><td class="cost text-right"><input type="hidden" name="attr_cost[]" value="' . $option->cost . '"><span>' . $option->cost . '</span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="' . $option->price . '"><span>' . $option->price . '</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>';
                                            }
                                        }
                                        ?></tbody>
                                </table>
...
<script type="text/javascript">
   ...
   img.src    = 'images/'+result[i];
        ...
        $(document).on('click', '#updateAttr', function () {
            ...
            row.children().eq(3).html('<input type="hidden" name="attr_cost[]" value="' + currencyFormate($('#acost').val()) + '"><span>' + currencyFormate($('#acost').val()) + '</span>');
            row.children().eq(4).html('<input type="hidden" name="attr_price[]" value="' + currencyFormate($('#aprice').val()) + '"><span>' + currencyFormate($('#aprice').val()) + '</span>');
            $('#aModal').modal('hide');
        });
    });
