...
function loadItems() {
    ...
    if (localStorage.getItem('reitems')) {
        ...
        $.each(reitems, function () {
            ...
            var item_type = item.row.type, combo_items = item.combo_items, sale_item_id = item.row.sale_item_id, item_option = item.row.option, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            ...
            tr_html = '<td><input name="sale_item_id[]" type="hidden" class="rsiid" value="' + sale_item_id + '"><input name="product_id[]" type="hidden" class="rid" value="' + item_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="option_id[]" type="hidden" class="roption" value="' + item_option + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')</span>'+(item_option != 0 ? ' - '+item.option.attribute : '')+'</td>';
...
<?php } ?>
                        ...
                        if($inv->payment_status == 'paid') {
                            echo '<div class="alert alert-success">'.sprintf(lang('payment_status_paid'), $inv->payment_status, $inv->paid).'</div>';
                        } else {
                            echo '<div class="alert alert-warning">'.sprintf(lang('payment_status_not_paid'), $inv->payment_status, $inv->paid).'</div>';
                        }
...
<?php echo form_textarea('note', (isset($_POST['note']) ? $_POST['note'] : ""), 'class="form-control" id="renote" required="required" style="margin-top: 10px; height: 100px;"'); ?> 
