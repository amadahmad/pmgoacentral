...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#date").datepicker({
            format: "<?=$this->dateFormats['js_sdate']?>",
            autoclose: true
        });
...
if ($Settings->discount_option == 2) {
    ...
    foreach ($discounts as $discount) {
        ...
        if ($discount->id == $Settings->default_discount) {
...
?><td><input class="input-block-level text-center" name="quantity' + count + '" type="text" value="1" id="quantity-' + count + '" onClick="this.select();"></td><td><input class="span2 tran" style="text-align:right;" name="unit_price' + count + '" type="text" id="price-' + count + '" value="' + item_price + '"></td><td><i class="icon-trash tip del" id="' + count + '" title="Remove this Item" style="cursor:pointer;" data-placement="right"></i></td>');
                ...
                count++;
                an++;
<?php if ($Settings->discount_option == 2 && $Settings->discount_method == 1) { ?>
                                        if (discount_type2 == 2) {
...
<?php } ?>
    ...
    <?php if ($Settings->discount_option == 2 && $Settings->discount_method == 2) { ?>
                                        if (discount_type2 == 2) {
                                            pr_discount = discount2;
                                        }
                                        if (discount_type2 == 1) {
...
<?php if ($Settings->discount_method == 1 && $Settings->discount_option == 1) { ?>
                                    if (discount_type == 2) {
...
<?php if ($Settings->discount_method == 2 && $Settings->discount_option == 1) { ?>
                                    if (discount_type == 2) {
                                        total_discount = discount;
                                    }
                                    if (discount_type == 1) {
...
<?php if ($Settings->discount_option == 1 || $Settings->discount_option == 2) { ?>$('#tds').val(total_discount.toFixed(2));<?php } ?>
<?php if ($Settings->tax1) { ?>$('#ttax1').val(total_tax1.toFixed(2));<?php } ?>
<?php if ($Settings->tax2) { ?>$('#ttax2').val(total_tax2.toFixed(2));<?php } ?>
...
<?php if ($Settings->discount_option == 2) { ?>
    ...
    <?php if ($Settings->discount_method == 1) { ?>
    ...
    <?php } elseif ($Settings->discount_method == 2) { ?>
                                    if (new_discount_type == 2) {
                                        new_pr_discount = new_discount_rate;
                                    }
                                    if (new_discount_type == 1) {
...
<?php } elseif ($Settings->discount_method == 2 && $Settings->discount_option == 1) { ?>
                                if (discount_type == 2) {
                                    new_discount_value = discount;
                                }
                                if (discount_type == 1) {
...
<?php if ($Settings->tax2) { ?>$('#ttax2').val(total_tax2.toFixed(2));<?php } ?>
                        ...
                        $("#name").autocomplete({
                            source: function(request, response) {
                                $.ajax({url: "<?php echo site_url('sales/suggestions'); ?>",
                                    data: {<?=$this->security->get_csrf_token_name(); ?>: '<?=$this->security->get_csrf_hash() ?>', term: $('#name').val()},
                                    dataType: 'json',
                                    type: 'get',
                                    success: function(data) {
                                        response(data);
...
<?php if ($Settings->tax2) { ?>
 ...
 <?php if ($Settings->discount_option == 1 && $Settings->discount_method == 1) { ?> 
...
<?php } ?>
    ...
    <?php if ($Settings->discount_option == 1 && $Settings->discount_method == 2) { ?> total_discount = ((total + total_tax1 + total_tax2) * discount) / 100;
                                    $('#tds').val(total_discount.toFixed(2));
<?php } ?>
...
<?php if ($Settings->discount_option == 1 && $Settings->discount_method == 2) { ?>
                                    var d1 = $('#discount_s').val();
                                    $.each(discounts, function() {
                                        if (this.id == d1) {
                                            new_discount_rate = parseFloat(this.discount);
...
<?php } ?>
	...
	<?php if ($Settings->tax2 && $Settings->discount_option == 2 && $Settings->discount_method == 1) { ?>
	    var invt = $('#tax2_s').val();
	    $.each(tax_rates, function() {
                                    if (this.id == invt) {
                                        new_tax_rate = parseFloat(this.rate);
...
<?php if ($Settings->discount_option == 1) { ?>
    ...
    <?php } ?>
    <?php if ($Settings->discount_method == 2) { ?>
                                    if (new_discount_type == 1) {
...
<div class="control-group">
    ...
    <div class="controls"> <?php echo form_input($date, (isset($_POST['date']) ? $_POST['date'] : ""), 'class="form-control input-tip" id="date" required="required"'); ?></div>
</div>
<div class="control-group">
    <label class="control-label" for="reference_no"><?php echo $this->lang->line("reference_no"); ?></label>
    <div class="controls"> <?php echo form_input('reference_no', (isset($_POST['reference_no']) ? $_POST['reference_no'] : $rnumber), 'class="form-control input-tip" id="reference_no" required="required"'); ?> </div>
</div>
<div class="control-group">
    <label class="control-label" id="warehouse"><?php echo $this->lang->line("warehouse"); ?></label>
    <div class="controls">  <?php
        $wh[''] = '';
        foreach ($warehouses as $warehouse) {
            $wh[$warehouse->id] = $warehouse->name;
        }
        echo form_dropdown('warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : $Settings->default_warehouse), 'id="warehouse" class="form-control input-tip" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("warehouse") . '" required="required" ');
        ?> </div>
</div>
<div class="control-group">
    ...
    <div class="controls">  <?php
        ...
        echo form_dropdown('biller', $bl, (isset($_POST['biller']) ? $_POST['biller'] : ""), 'id="biller" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("biller") . '" required="required" class="form-control input-tip"');
        ?> </div>
</div>
<div class="control-group">
    <label class="control-label" id="customer"><?php echo $this->lang->line("customer"); ?></label>
    <div class="controls">  <?php
        $cu[""] = "";
        ...
        echo form_dropdown('customer', $cu, (isset($_POST['customer']) ? $_POST['customer'] : ""), 'id="customer" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("customer") . '" required="required" class="form-control input-tip"');
        ?> </div>
</div>
<?php if ($Settings->tax2) { ?>
    <div class="control-group">
        <label class="control-label" id="tax2"><?php echo $this->lang->line("tax2"); ?></label>
        <div class="controls">  <?php
    $tr[""] = "";
    foreach ($tax_rates as $tax) {
        $tr[$tax->id] = $tax->name;
    }
    echo form_dropdown('tax2', $tr, (isset($_POST['tax2']) ? $_POST['tax2'] : $Settings->default_tax_rate2), 'id="tax2" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("tax2") . '" required="required" class="form-control input-tip"');
    ?> </div>
    </div>
<?php } ?>
<?php if ($Settings->discount_option == 1) { ?>
    ...
    echo form_dropdown('inv_discount', $ds, (isset($_POST['inv_discount']) ? $_POST['inv_discount'] : $Settings->default_discount), 'id="discount_s" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("discount") . '" required="required" class="form-control input-tip"');
    ?> </div>
    </div>
<?php } ?>
...
<?php echo form_textarea('note', (isset($_POST['note']) ? $_POST['note'] : ""), 'class="input-block-level" id="note" style="margin-top: 10px; height: 100px;"'); ?> 
    ...
    <?php if ($Settings->discount_option == 1 || $Settings->discount_option == 2) { ?>
