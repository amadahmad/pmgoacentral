...
function loadItems() {
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function () {
            ...
            var item_cost = item.row.cost, item_qty = item.row.qty, item_expiry = item.row.expiry, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_code = item.row.code, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
