...
class Customers extends MY_Controller {
    ...
    function getCustomers() {
        ...
        $this->datatables
                ->select("id, company, name, phone, email, city, country, customer_group_name")
                ->from("companies")
    ...
    function import_csv() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) /* if($_FILES['userfile']['size'] > 0) */ {
                ...
                foreach ($final as $csv) {
                    if ($this->companies_model->getCustomerByEmail($csv['email'])) {
        ...
        if ($this->form_validation->run() == true && $this->companies_model->add_customers($final)) {
