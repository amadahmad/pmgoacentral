...
class Auth extends MY_Controller {
    ...
    function delete_avatar($id = NULL, $avatar = NULL) {
        ...
        if (!$this->ion_auth->logged_in() || !$this->ion_auth->in_group(array('super-admin', 'admin')) && $id != $this->session->userdata('user_id')) {
            $this->session->set_flashdata('warning', lang("access_denied"));
            redirect($_SERVER["HTTP_REFERER"]);
        } else {
    ...
    function login($m = NULL) {
        ...
        if ($this->form_validation->run() == true) {
          ...
          if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
            ...
            if($m) { $this->data['error'] = lang('we_are_sorry_as_this_sction_is_still_under_development.'); }
    ...
    function change_password() {
        ...
        $this->form_validation->set_rules('new', lang('new_password'), 'required||max_length[8]|matches[new_password_confirm]');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $bc = array(array('link' => site_url('home'), 'page' => lang('home')), array('link' => site_url('auth/users'), 'page' => lang('user')), array('link' => '#', 'page' => lang('change_password')));
            $meta = array('page_title' => lang('users'), 'bc' => $bc);
            $this->page_construct('auth/profile/'.$id.'#cpassword', $meta, $this->data);
        } else {
    ...
    if ($this->form_validation->run() === TRUE) {
        ...
        if ($this->Owner) {
            ...
            if ($this->input->post('password')) {
                $this->form_validation->set_rules('password', lang('edit_user_validation_password_label'), 'required|min_length[' . $this->config->item('min_password_length', 'auth') . ']|max_length[' . $this->config->item('max_password_length', 'auth') . ']|matches[password_confirm]');
...
function register() {
    ...
    $this->form_validation->set_rules('password', lang('password'), 'required|min_length[' . $this->config->item('min_password_length', 'auth') . ']|max_length[' . $this->config->item('max_password_length', 'auth') . ']|matches[password_confirm]');
