...
class Settings_model extends CI_Model {
    ...
    public function getAttributeByID($id) {
        $this->db->select("attributes.id as id, title, type, group_concat(concat(attribute_options.option) SEPARATOR ', ') as options")
            ->join('attribute_options', 'attributes.id=attribute_options.attribute_id', 'left')
            ->group_by("attributes.id");
        $this->db->order_by('attribute_options.id');
        $q = $this->db->get_where('attributes', array('attributes.id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function addAttribute($data) {
        ...
        if ($this->db->insert("attributes", $attribute)) {
            $attribute_id = $this->db->insert_id();
            if($data['type'] != 'text' && $data['options']) {
                foreach($data['options'] as $option){
                    if(isset($option) && !empty($option)){
                        $this->db->insert("attribute_options", array('attribute_id' => $attribute_id, 'option' => $option));
                    }
                }
            }
            return true;
    ...
    public function updateAttribute($id, $data = array()) {
        $attribute = array(
            'title' => $data['title'],
            'type' => $data['type']
        );
        if ($this->db->update("attributes", $attribute, array('id' => $id)) && $this->db->delete("attribute_options", array('attribute_id' => $id))) {
            if($data['type'] != 'text' && $data['options']) {
                foreach($data['options'] as $option){
                    if(isset($option) && !empty($option)){
                        $this->db->insert("attribute_options", array('attribute_id' => $id, 'option' => $option));
                    }
                }
            }
            return true;
    ...
    public function deleteAttribute($id) {
        if ($this->db->delete("attributes", array('id' => $id))) {
            return true;
        }
        return FALSE;
    }*/
